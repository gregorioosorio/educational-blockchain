...
class Transaction {
    ...
    calculateHash() {
        return crypto
            .createHash('sha256')
            .update(this.fromAddress + this.toAdress + this.amount + this.sequence + this.timestamp)
            .digest('hex');
    }
...
class Block {
    ...
    calculateHash() {
        return crypto
            .createHash('sha256')
            .update(JSON.stringify(this.transactions).toString() + this.previousHash + this.salt + this.timestamp)
            .digest('hex');
    }
